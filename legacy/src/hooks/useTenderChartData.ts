...
export function useTenderChartData(
  ...
  const generateDateRange = useCallback(
    (dates: string[], unit: "year" | "month") => {
      if (dates.length === 0) return [];

      const minDate = Math.min(
        ...dates.map((d) => parseInt(d.substring(0, 6)))
      );
  ...
  const processChartData = useCallback(() => {
    ...
    try {
      ...
      // 先過濾得標案件
      const winningTenders = tenders.filter(
        (t) =>
          t.status === "得標" && !processedTenders.has(t.tenderId) && t.date
      );

      // 根據時間單位過濾資料
      const filteredWinningTenders =
        timeUnit === "month"
      ...
      filteredWinningTenders.forEach((tender) => {
        ...
        const key =
          timeUnit === "month" ? date.substring(0, 6) : date.substring(0, 4);
