...
// 創建提供者組件
export function FieldRendererProvider({ children }: { children: React.ReactNode }) {
...
export default function FieldRenderer({ field, depth = 0, sectionTitle, parentKey = '' }: FieldRendererProps) {
  ...
  // 處理陣列型態的複雜資料結構
  const isArrayOfObjects = Array.isArray(field.value) && field.value.some((item: any) => typeof item === 'object');
