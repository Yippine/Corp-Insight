...
const formatSearchData = (company: SearchResponse): SearchData => {
  return {
    ...
    totalCapital: formatCapital(
      company["資本額(元)"] ?? company["資本總額(元)"]
    ),
...
const formatDetailData = (
  ...
  return {
    ...
    status: getCompanyStatus(company),
    chairman: company.負責人姓名 || company.代表人姓名 || "無",
    established: formatDate(
      company.核准設立日期 || { year: 0, month: 0, day: 0 }
    ),
    lastChanged: formatDate(
      company.最後核准變更日期 || { year: 0, month: 0, day: 0 }
    ),
    address: company.地址 || company.公司所在地 || "未提供",
    phone: "未提供",
    email: "未提供",
