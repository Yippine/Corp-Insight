 ...
 * @returns 格式化後的公司詳細資料
 */
export function formatDetailData(taxId: string, data: CompanyResponse): CompanyData {
  // 從原始資料抽取所需資訊
  return {
    name: getCompanyName(data),
    taxId: taxId,
    ...
    lastChanged: formatDate(data.最後核准變更日期 || { year: 0, month: 0, day: 0 }),
    address: data.地址 || data.公司所在地 || '未提供',
    phone: '未提供',
    email: '未提供',
    website: '未提供',
...
export async function formatCompanyResults(type: 'taxId' | 'name' | 'chairman', data: any): Promise<CompanyData[]> {
 ...
 * 格式化董事資料
 */
function formattedDirectors(company: CompanyResponse): { name: string; title: string; shares: string; representative: string | [number, string] }[] {
