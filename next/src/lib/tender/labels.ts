export function getCompanyLabel(record: any, searchQuery: string): string {
  const companyName =
...
export function getTenderLabel(type: string): string {
  const labelPatterns = [
    {
      label: '已決標',
...
export function getLabelStyle(label: string): string {
  ...
  return `${baseStyle} ${colorStyles[label] || 'bg-gray-100 text-gray-800'}`;
}
