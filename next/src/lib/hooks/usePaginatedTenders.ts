...
export function usePaginatedTenders(taxId: string) {
      ...
      try {
        ...
        return {
          tenders: data.records.map((record: any) => {
            ...
            if (companies) {
              const idKeyEntry = Object.entries(companies.id_key || {}).find(
                ([id]) => id === taxId
              );
              if (idKeyEntry) {
                ...
                if (companyIndex !== -1) {
                  ...
                  if (companyName) {
                    ...
                    if (
                      Array.isArray(nameKeyStatus) &&
                      !nameKeyStatus.some(s => s.includes('未得標'))
                    ) {
                      status = '得標';
                    }
                  }
        ...
        if ((error as Error).name === 'AbortError') {
          ...
          console.error(
            `載入第 ${page} 頁標案資料失敗 (taxId: ${taxId})：`,
            error
          );
          if (!((error as Error).name === 'AbortError')) {
            setError(
              error instanceof Error ? error.message : '載入標案資料時發生錯誤'
            );
          }
        }
        throw error;
      ...
      try {
        ...
        if (firstPage.totalPages > 1) {
          ...
          for (let page = 2; page <= firstPage.totalPages; page += batchSize) {
            if (signal.aborted) throw new DOMException('Aborted', 'AbortError');
            console.log(
              `開始載入第 ${page} 到第 ${Math.min(page + batchSize - 1, firstPage.totalPages)} 頁 (taxId: ${taxId})`
            );
            const batchPromises = [];
            for (
              let i = 0;
              i < batchSize && page + i <= firstPage.totalPages;
              i++
            ) {
            ...
            console.log(
              `目前已載入 ${currentTendersArray.length} 筆標案資料 (taxId: ${taxId})`
            );
            setTenders(currentTendersArray);
            setCurrentPage(
              Math.min(page + batchSize - 1, firstPage.totalPages)
            );
        ...
        if (signal.aborted) {
          console.log(
            `loadAllPages for taxId ${taxId} was aborted. isLoadingMore will be handled by effect cleanup.`
          );
        } else {
  ...
  useEffect(() => {
    ...
    loadAllPages(signal).catch(error => {
      if ((error as Error).name === 'AbortError') {
        console.log(
          `loadAllPages call was directly aborted for taxId: ${taxId}`
        );
      } else {
        console.error(
          'Unhandled error in loadAllPages effect promise: ',
          error
        );
      }
    });
    ...
    return () => {
      console.log(
        `清理 usePaginatedTenders effect (taxId: ${taxId})，中止請求...`
      );
  ...
  return {
    ...
    error,
    progress:
      totalPages > 0 && currentPage > 0 ? (currentPage / totalPages) * 100 : 0,
    totalPages,
    currentPage,
    isFullyLoaded,
  };
