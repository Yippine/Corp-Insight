 ...
 */
export async function getCachedApiData<T = any>(collectionName: string, apiKey: string): Promise<T | null> {
  try {
    const db = await getDb();
    const collection = db.collection(collectionName);
    const cachedItem = await collection.findOne({ api_key: apiKey, expires_at: { $gt: new Date() } });
    ...
    console.error(`MongoDB: Error getting cached data from "${collectionName}" for key "${apiKey}"`, error);
 ...
 */
export async function setCachedApiData(collectionName: string, apiKey: string, data: any, ttlSeconds: number): Promise<void> {
  try {
    const db = await getDb();
    const collection = db.collection(collectionName);
    const now = new Date();
    ...
    console.error(`MongoDB: Error setting cached data in "${collectionName}" for key "${apiKey}"`, error);
  }
}
...
export async function logEmailVerification(
  ...
  try {
    ...
    console.error(`MongoDB: Error logging email verification for ${email}`, error);
  }
}
 ...
 */
export async function logFeedbackSubmission(submissionData: any): Promise<void> {
  try {
    const db = await getDb();
...
// Optional: Close connection when app shuts down (more relevant for standalone scripts)
...
// Basic test function (can be removed or run separately)
// async function testMongoDbUtils() {
//   try {
//     console.log('Testing MongoDB Utils...');
//     await getDb(); // Test connection

//     const testApiKey = 'test_api_key_123';
//     const testCollection = 'pcc_api_cache'; // Use an existing cache collection for testing

//     // Test set
//     await setCachedApiData(testCollection, testApiKey, { message: 'Hello MongoDB Cache!' }, 60);

//     // Test get
//     const cachedData = await getCachedApiData(testCollection, testApiKey);
//     console.log('Retrieved from cache:', cachedData);

//     // Test get expired (wait for it or set a very short TTL for immediate test)
//     // await new Promise(resolve => setTimeout(resolve, 61000)); 
//     // const expiredData = await getCachedApiData(testCollection, testApiKey);
//     // console.log('Retrieved expired data (should be null):', expiredData);

//     // Test logging
//     await logEmailVerification('test@example.com', 'test_jwt_token', true, new Date(), new Date(Date.now() + 600000));
//     await logFeedbackSubmission({ type: 'test', title: 'Test Feedback', content: 'This is a test.', email: 'test@example.com' });

//     console.log('MongoDB Utils test completed.');
//   } catch (error) {
//     console.error('Error during MongoDB Utils test:', error);
//   } finally {
//     if (client) {
//       await client.close();
//       console.log('MongoDB: Test client connection closed');
//     }
//   }
// }

// Uncomment to run test when this file is executed directly (e.g., node mongodbUtils.js)
