...
import { Terminal, Play, StopCircle, Loader2, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function TaskRunnerCard({ title, description, scriptName }: TaskRunnerCardProps) {
  ...
  const runScript = async () => {
    ...
    try {
      ...
      while (true) {
        ...
        const exitCodeMatch = chunk.match(/--- Process finished with exit code (\d+) ---/);
        if (exitCodeMatch) {
  ...
  const getStatusIcon = () => {
    ...
    if (exitCode === 0) return <CheckCircle className="h-5 w-5 text-green-500" />;
    if (exitCode !== null && exitCode !== 0) return <AlertTriangle className="h-5 w-5 text-red-500" />;
    ...
    <div className="flex flex-col rounded-lg border border-gray-200 bg-white shadow-sm transition-all hover:shadow-md">
      ...
      {output && (
        <div className="mx-4 mb-4 max-h-60 overflow-y-auto rounded-md bg-gray-900 p-4 font-mono text-xs text-white">
            <pre className="whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: ansiToHtml(JSON.stringify(output)) }}></pre>
        </div>
      )}
      ...
      <div className="flex items-center justify-between rounded-b-lg bg-gray-50 px-6 py-4">
        <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-gray-500">
                {isRunning ? '執行中...' : (exitCode !== null ? `已結束 (Code: ${exitCode})` : '待命中')}
            </span>
        </div>
        <button
          onClick={runScript}
          disabled={isRunning}
          className="inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 bg-blue-600 hover:bg-blue-700 focus:ring-blue-500"
        >
