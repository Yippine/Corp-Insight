...
export const dynamicTitles = {
  // 公司搜尋
  companySearchResult: (query: string) => `搜尋：「${query}」 | ${COMPANY_TITLE} | ${SITE_TITLE}`,
  companySearchNoResult: (query: string) => `查無「${query}」資料 | ${COMPANY_TITLE} | ${SITE_TITLE}`,
  
  // 公司詳情
  companyDetail: (companyName: string) => `${companyName} | ${COMPANY_TITLE} | ${SITE_TITLE}`,
  
  // 標案搜尋
  tenderSearchWithQueryAndType: (query: string, searchType: 'company' | 'tender') => `${searchType === 'company' ? '廠商' : '標案'}：「${query}」 | ${TENDER_TITLE} | ${SITE_TITLE}`,
  tenderSearchNoResult: (query: string, searchType: 'company' | 'tender') => `查無「${query}」${searchType === 'company' ? '廠商' : '標案'} | ${TENDER_TITLE} | ${SITE_TITLE}`,
  
  // 標案詳情
  tenderDetailWithName: (tenderName: string) => `${tenderName} | ${TENDER_TITLE} | ${SITE_TITLE}`,
  
  // AI工具搜尋
  aiToolSearchWithQuery: (query: string) => `搜尋：「${query}」 | ${AI_TOOL_TITLE} | ${SITE_TITLE}`,
  aiToolSearchWithTag: (tag: string) => `類別：「${tag}」 | ${AI_TOOL_TITLE} | ${SITE_TITLE}`,
  aiToolSearchWithQueryAndTag: (query: string, tag: string) => `搜尋：「${query}」、類別：「${tag}」 | ${AI_TOOL_TITLE} | ${SITE_TITLE}`,
  aiToolSearchNoResult: (query?: string, tag?: string) => {
    let searchTerm = '';
    if (query) searchTerm += `「${query}」關鍵字`;
    if (tag) searchTerm += `${tag ? '、' : ''}「${tag}」類別`;
    return searchTerm ? `查無${searchTerm} | ${AI_TOOL_TITLE} | ${SITE_TITLE}` : `查無 AI 助理 | ${AI_TOOL_TITLE} | ${SITE_TITLE}`;
  },
  
  // AI工具詳情
  aiToolDetail: (toolName: string) => `${toolName} | ${AI_TOOL_TITLE} | ${SITE_TITLE}`,
};

// 為了向後兼容，提供一個生成標題的輔助函數
export function generatePageTitle(base: string, suffix: string): string {
  return suffix ? `${base} | ${suffix} | ${SITE_TITLE}` : `${base} | ${SITE_TITLE}`;
}