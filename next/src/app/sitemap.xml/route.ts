...
export async function GET() {
  ...
  try {
    ...
    // 獲取熱門企業（基於搜索頻率或其他指標）
    const popularCompanies = await db.collection('companies')
      .find({})
    ...
    // 獲取重要標案
    const importantTenders = await db.collection('tenders')
      .find({})
    ...
    // 獲取熱門 AI 工具
    const popularAiTools = await db.collection('ai_tools')
      .find({})
