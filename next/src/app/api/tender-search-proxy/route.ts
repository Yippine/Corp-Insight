...
export async function GET(request: NextRequest) {
  ...
  if (!taxId) {
    return NextResponse.json({ error: 'Missing taxId parameter' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch tender search data', details: errorMessage }, { status: 500 });
  }
}
