...
export async function GET(request: NextRequest) {
  ...
  if (!unitId || !jobNumber) {
    return NextResponse.json({ error: 'Missing unit_id or job_number parameter' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`External API error! status: ${response.status} for ${externalApiUrl}`);
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch tender detail data', details: errorMessage }, { status: 500 });
  }
}
