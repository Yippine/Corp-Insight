...
const collectionInitialState: { name: string; description: string }[] = [
  ...
  { name: 'prompt_templates', description: '全域設定' },
];
...
export function useDatabaseStatus() {
  const [collectionDetails, setCollectionDetails] =
    useState<CollectionStats[]>(createInitialStatus);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const updatedDetails = createInitialStatus().map(initialState => {
        const detail = data.collectionDetails?.find(
          (d: CollectionStats) => d.name === initialState.name
        );
        if (detail) {
          return {
            ...initialState,
      ...
      const formattedStats: DatabaseOverallStats = {
        ...
        latestBackupDate: data.latestBackupDate
          ? new Date(data.latestBackupDate).toLocaleString('zh-TW')
          : '無',
