...
async function main() {
  ...
  try {
    ...
    for (const collection of collectionsToBackup) {
      ...
      const sourcePathOnHost = path.join(__dirname, '..', 'db', `${collection}.json`);
      ...
      if (!fileExists) {
        throw new Error(`Timeout: Exported file ${sourcePathOnHost} did not appear in time.`);
      }
    ...
    console.log(colorize('[App Container] ✅ 所有 Collections 匯出完成', 'green'));
