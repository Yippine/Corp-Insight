...
function main() {
    ...
    files.forEach((file, index) => {
        ...
        console.log(`   📏 檔案大小: ${colorize(formatBytes(analysis.fileSize), 'cyan')}`);
        
        if (analysis.error) {
            console.log(`   ❌ 錯誤: ${colorize(analysis.error, 'red')}`);
        } else {
            console.log(`   📝 資料筆數: ${colorize(analysis.recordCount.toString(), 'green')}`);
            
            if (analysis.toolTypes && analysis.toolTypes.length > 0) {
                console.log(`   🔧 工具類型: ${colorize(analysis.toolTypes.join(', '), 'yellow')}`);
            }
            
            if (analysis.categories && analysis.categories.length > 0) {
                console.log(`   📂 分類: ${colorize(analysis.categories.join(', '), 'yellow')}`);
            }
            
            if (analysis.sampleData) {
                const sampleKeys = Object.keys(analysis.sampleData);
                console.log(`   🔍 資料欄位: ${colorize(sampleKeys.slice(0, 5).join(', '), 'cyan')}${sampleKeys.length > 5 ? '...' : ''}`);
            }
        }
    ...
    // 顯示摘要統計
    const validAnalyses = files.map(file => analyzeBackupFile(path.join(BACKUP_DIR, file)))
        .filter(analysis => !analysis.error);
    
    if (validAnalyses.length > 0) {
        const totalRecords = validAnalyses.reduce((sum, analysis) => sum + analysis.recordCount, 0);
        ...
        console.log(`   工具類型總數: ${colorize(allToolTypes.size.toString(), 'yellow')}`);
        console.log(`   分類總數: ${colorize(allCategories.size.toString(), 'yellow')}`);
    }
